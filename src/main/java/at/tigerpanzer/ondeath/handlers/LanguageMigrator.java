...
public class LanguageMigrator {
  ...
  public static void configUpdate() {
    ...
    Bukkit.getConsoleSender().sendMessage(Utils.color("§7[§eOnDeath§7] =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-="));
    Bukkit.getConsoleSender().sendMessage(ChatColor.YELLOW + "[OnDeath] System notify >> Your config file is outdated! Updating...");
    ...
    Bukkit.getConsoleSender().sendMessage(ChatColor.GREEN + "[OnDeath] [System notify] You're using latest config file version! Nice!");
    Bukkit.getConsoleSender().sendMessage(Utils.color("§7[§eOnDeath§7] =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-="));
  }
  ...
  public static void languageFileUpdate() {
    ...
    Bukkit.getConsoleSender().sendMessage(Utils.color("§7[§eOnDeath§7] =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-="));
    Bukkit.getConsoleSender().sendMessage(ChatColor.YELLOW + "[OnDeath] [System notify] Your language file is outdated! Updating...");
    ...
    Bukkit.getConsoleSender().sendMessage(ChatColor.GREEN + "[OnDeath] [System notify] Language file updated! Nice!");
    Bukkit.getConsoleSender().sendMessage(ChatColor.GREEN + "[OnDeath] [System notify] You're using latest language file version! Nice!");
    Bukkit.getConsoleSender().sendMessage(Utils.color("§7[§eOnDeath§7] =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-="));
  }
  ...
  public static void migrateToNewFormat() {
    ...
    Bukkit.getConsoleSender().sendMessage(ChatColor.GREEN + "OnDeath is migrating all files to the new file format...");
    ...
    for (String file : migratable) {
      if (Utils.getFile(plugin, file).exists()) {
        Utils.getFile(plugin, file).renameTo(new File(plugin.getDataFolder(), "OnDeath_" + file + ".yml"));
        Bukkit.getConsoleSender().sendMessage(ChatColor.GREEN + "Renamed file " + file + ".yml");
      }
    }
    Bukkit.getConsoleSender().sendMessage(ChatColor.GREEN + "Done! Enabling OnDeath...");
    Bukkit.getConsoleSender().sendMessage(Utils.color("§7[§eOnDeath§7] =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-="));
  }
